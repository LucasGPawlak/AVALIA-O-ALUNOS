...
declare
  ...
  n text;
begin
  insert into turmas (nome, ano) values ('DS3', 2026), ('DS2', 2026), ('DS1', 2026)
  on conflict do nothing;

  select id into t_ds3 from turmas where nome = 'DS3' limit 1;
  select id into t_ds2 from turmas where nome = 'DS2' limit 1;
  select id into t_ds1 from turmas where nome = 'DS1' limit 1;

  foreach n in array ds1 loop
    if not exists (select 1 from estudantes where nome = n and turma_id = t_ds1) then
      insert into estudantes (nome, turma_id) values (n, t_ds1);
    end if;
  end loop;
